...
| ✅ Terminal renderer picker | Appearance → Terminal exposes the three-way webgl/canvas/dom picker on macOS (GH #140); driving the real `<select>` lands in prefs and keeps the legacy `terminalGpuEnabled` in sync | `settings.e2e.ts` |
