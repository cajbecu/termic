...
| ✅ Git history / graph | The Commit tab's Graph section lists real commits (made outside the app) newest first with lane gutters + ref chips; a commit expands into its files; a file opens a diff of THAT revision (asserted against a deliberately dirtied working tree) with no review affordances; the ref picker scopes to Auto, All and a named branch, and unticking the last ref returns to Auto; subjects indent to their own lane (GH #199, GH #208) | `git.e2e.ts` |
