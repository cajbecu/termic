...
`src-tauri/src/docker.rs` + Settings → Docker Sandbox (`DockerSection.tsx`) + `SandboxEngineSelector.tsx` (`NewTaskDialog.tsx` and `TaskSandboxDialog.tsx`, see "Unified engine selector" below). A mutually-exclusive alternative to Seatbelt: the agent CLI runs inside `docker run` instead, and can only touch what termic bind-mounts. See `docs/plans/docker-sandbox/` for the full design + research.

- **Unified engine selector**: `SandboxEngineSelector.tsx` renders three top-level cards - Off / macOS Seatbelt / Docker Container - shared by both `NewTaskDialog` (task creation) and `TaskSandboxDialog` (editing an existing task). Docker used to be reachable ONLY from the edit dialog, after a task already existed; it's now offered at creation too (`docker_sandbox_enabled` on `CreateTaskArgs`/`CreateMultiArgs`/`task_open_repo`/`task_import_worktree`, see the Gating bullet above). Picking "macOS Seatbelt" reveals `SandboxModeSelector` (`hideOff`) below it for the Monitor / Enforcing (FS) / Enforcing sub-choice - `SandboxMode` itself is unchanged (still 4 values incl. `off`); the engine selector is a UI layer over the SAME two underlying fields (`sandbox_mode` and `docker_sandbox_enabled`), not a new data model. In `TaskSandboxDialog` the two engines keep their pre-existing, independent commit paths: Docker still commits IMMEDIATELY through its own confirm (`toggleDocker`/`task_set_docker`, always SIGKILLs), while Seatbelt stays a draft the Save button commits (`taskSetSandbox`/`task_set_sandbox`, restart-or-not choice) - `chooseEngine` just decides which of the two a click should drive, rather than merging them into one commit. Picking "Docker Container" always shows the `<u>network is unrestricted for now</u>` note (`DockerEngineNote`, same copy as Settings → Docker Sandbox) and, in the edit dialog, the "Preview command" toggle.
- **Docker's own status icon**: `DockerSandboxIcon` (`SandboxIcon.tsx`, next to `SANDBOX_VISUALS`/`SandboxIcon`) is a `Container` glyph colored `DOCKER_SANDBOX_COLOR` (`var(--color-err)`) - the same warning red `Sidebar.tsx` already uses for "YOLO on but the cage isn't actually enforced", not a color of its own. Deliberate: Docker's cage never reaches the green "real cage" state Seatbelt's `enforce` mode does (filesystem-only, network stays open), so it never gets to wear green. Because Docker mode always stores `sandbox_mode` as `off` (mutually exclusive with Seatbelt), every surface that used to key off `effectiveSandboxMode(task) !== "off"` to decide whether to show a sandbox badge had to add an explicit `task.docker_sandbox_enabled` check FIRST - otherwise a Docker-sandboxed task showed no badge at all, reading as unsandboxed. Wired into the same four places `SandboxIcon` already appeared: the sidebar row badge + its task-menu item (`Sidebar.tsx`), the terminal footer chip (`TerminalPane.tsx`'s `FooterBar`), the top toolbar badge (`UnifiedBar.tsx`), and the command palette's "Sandbox settings" suffix (`CommandPalette.tsx`, text-only there - no icon to swap).
