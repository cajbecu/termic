...
- **Anything the skipped dialog used to ask, the pref has to answer.** Archiving is the case with a second question: the dialog's delete-branch checkbox has no silent equivalent, so ticking "Don't ask again" freezes that answer into `archiveDeleteBranch` and every silent archive reuses it (never for a main checkout, which has no worktree branch and never showed the checkbox). Settings › Tasks exposes both prefs so a stored "yes, delete the branch" is visible and reversible.
