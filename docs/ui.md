...
Both queue into the same `reviewComments` store keyed by file, so editor and diff remarks batch together and the pending-comments bar sends them as ONE message.
