...
//   - task  (broadcastForTaskId)      → every live agent/shell tab of ONE
//                                           task.
//   - project    (broadcastForProjectId) → the MAIN agent (the default,
//                                           main-pane tab) of every task
//                                           in the project.
...
/** One broadcast destination: a live-PTY tab plus the task it belongs to
 *  (project mode spans tasks, so the taskId can't be a single dialog-level
 *  value) and the label to show for it. */
interface BTarget { tab: TerminalTab; taskId: string; label: string }
...
export function BroadcastDialog() {
  ...
  // Task mode keeps the old rule (agents on, raw shells / custom off).
