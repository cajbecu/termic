...
A git History tab, comments on any file you read, ripgrep search, and a big idle-CPU fix.

### Features
- The right panel gets a History tab: a commit graph of the task's branch, modelled on VS Code's Source Control Graph. A dense row per commit (lane gutter, ref chips, subject, age), click a row for the files it touched, click a file for the diff of that revision. Committed work used to vanish from Termic the moment the tree went clean, so watching what an agent had actually done meant leaving for VS Code or Fork. The staging tab is now called "Commit", since "Git" no longer said which of the two you were on. Lane colours come from the theme palette, so a custom theme recolours the graph. (#199)
- Select code in any file you are reading and comment on it, the same surface the diff pane has. The comment queues in place with the code quoted above the box, several remarks ship as one message, and a body is optional (the selected code alone is a legitimate message). The popover has two exits: Send ships that one comment now, Add to pending queues it with the rest. Shift+Cmd+L stacks a selection from the keyboard. A batch made while reading a file no longer opens with "I reviewed your changes", because it is not a review of anything the agent wrote. Queued comments follow their code: type three lines above one and it moves with the lines it points at, and the quote is frozen at capture, so a comment never ends up quoting code you never saw. (#174)
- Find in files runs on ripgrep when you have it. It is faster on large repos, Unicode-aware, and its regex is far closer to what the results highlight with, so the search and the highlighting agree. Without ripgrep everything falls back to `git grep` exactly as before, and the dialog names the backend that ran and offers an install hint only on the fallback. (#181)
- Find in files gets a `.*` regexp toggle and an `Aa` match-case toggle, both off by default. The count, the search indicator and the empty states merged into one fixed-height row, so the dialog no longer shifts while you type.
- Right-click any tab pill for Pin, Close, Close others, Close to the right. Pinning is an ordering operation: pinned tabs sit in their own block outside the scroller, so they stay in reach however far the strip is scrolled, and drag-to-reorder cannot break the boundary. A pinned pill trades its close X for an unpin control, since a pinned tab should not be one stray click from a dead PTY. The pinned flag persists, so a pinned agent tab comes back pinned and leftmost. (#183)
- The Unstaged and Staged sections in the Commit tab collapse. Stage all / Unstage all stay live while a section is collapsed, and the state persists across a restart.
- Right-clicking a task row in the sidebar now offers a New submenu with the same rows as the tab strip's "+", so adding a second agent or terminal to a task is findable from where people look for it. (#197)
- `termic://` links open Termic on a New Task dialog that is already filled in: `termic://new?project=web&worktree=1&name=fix-login&p=Fix%20the%20login%20bug`. The link never creates anything, it only pre-fills the form and a human still presses Create, which is the whole security model for accepting a prompt from any web page that can navigate to a URL scheme. New Task grows an optional first-message box, blank at every existing entry point. (#192)
- Set a default location for worktrees in Settings, Tasks, and override it per repository. An invalid or git-tracked path is refused at save time rather than at task creation.
- `termic tab close <task> --tab <sel>` closes a tab from the shell, the one tab verb that also reaches plain shells and custom terminals. Anything opening tabs programmatically can now clean up after itself. (#185)
- `termic prompts` lists your prompt library and `termic prompts show <sel>` prints one body. `-P/--library <sel>` on `new`, `send` and `tab` fires a library prompt, and with `-p` too the body arrives first and your text after it, which is the cross-agent handoff: `termic result plan | termic new review --agent codex -P builtin:review -p -`.
- The archive dialog gets "Don't ask again". Turning confirmation off moves the delete-branch choice into Settings, Tasks, so archiving still knows what to do about the branch. Note that with confirmation off, the command palette's Archive entry archives on Enter with no prompt; Settings, Tasks is the way back.

### Bug fixes
- Idle Termic no longer sits on a third of a core doing nothing. An agent TUI re-emits its unchanged window title while it waits at the prompt, and the setter that received it wrote to the store every time even though nothing had changed, which re-ran every selector in every open task. On a 16-terminal window, idle CPU drops from 19% to 12.7% and store writes halve. A second allocation bug in the script-run log, two array copies per line once the log hit its cap, is fixed alongside it. (#212)
- A slow login shell no longer costs a session its environment for the rest of the app's life. The probe result used to be cached even when it failed, so one transiently slow rc file meant every terminal opened after it got a bare fallback PATH until you restarted Termic. The probe now retries in the background with backoff and swaps the real environment in when it lands, so new sessions pick it up without a restart. (#186)
- Nix profile directories are on the fallback PATH, so a nix-darwin or home-manager machine no longer hides every nix-installed CLI from spawned sessions during the window where the fallback is in use. They lead the list, matching how a nix login shell resolves. CLI detection reads the same list, so the install badge and the actual spawn cannot disagree. (#187)
- Narrowing one of an agent's title patterns in Settings no longer deletes the other two. Setting a custom busy pattern used to swap out the whole built-in set for that agent, which silently killed the fast done signal: no error, no hint, the agent just stopped reporting done until the slower detectors caught up. Each field resolves on its own now. Emptying a field means inherit, so switching one off needs an unmatchable pattern like `(?!)`.
- In a multi-repo task the Git tab opens on a repo that actually changed, instead of sitting on the empty wrapper with no pill selected until you clicked one.
- Clicking a desktop notification only brings the window forward now. It used to jump you to the notification's task on any refocus within 15 seconds, so a plain Cmd+Tab back yanked you off whatever you were reading. The unread dot already marks where the work happened.
- The bottom terminal takes focus when you open it. The footer Terminal button, expanding a collapsed split, and clicking a shell pill all left focus behind, so you had to click the terminal before you could type. Collapsing the split hands focus back to the pane you came from instead of dropping it on the page body, which was swallowing every keystroke.
- A pinned tab holds its width instead of resizing on every title the agent emits and shoving the rest of the strip sideways.
- Settings, projects and task metadata are written atomically, so a crash or a power cut during a write cannot leave a truncated file where your configuration used to be.
