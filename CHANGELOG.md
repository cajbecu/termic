...
- The terminal renderer is a three-way choice in Appearance, Terminal, on every platform: GPU (WebGL), Canvas, or DOM. It used to be a WebGL on/off toggle that macOS hid entirely, so a Mac user had no way to change renderer without hand-editing localStorage. WebGL stays the default and is by far the cheapest once output is flowing. Canvas draws on the GPU without a live WebGL surface, so it costs less while terminals sit idle and more under heavy output, which suits a laptop left with idle terminals. DOM is the compatibility fallback for setups where the GPU path misbehaves, such as the Linux/WebKitGTK boxes that run WebGL on a software rasterizer. Applies to terminals opened after the change; relaunch to switch the ones already open. (#140)
