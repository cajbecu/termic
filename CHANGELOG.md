...
- The GPU (WebGL) terminal renderer toggle is available on macOS, where it was previously hidden behind a platform check. On macOS 26 a live WebGL surface has been measured carrying a large standing WindowServer and GPU cost even with the terminal idle, and not having the layer is the only lever against it. The default stays on, so nothing changes unless you turn it off. (#140)
